...
# 分类构件
class BaiYe(ClassifiedEntity):
    chinese_name = "百叶"

    def __init__(self, entity_object: Entity, border_entity: BorderEntity) -> None:
        ClassifiedEntity.__init__(self, entity_object)

        self.chinese_name = "百叶"
        ...
        # bgy属性
        self.window_number = window_numbering_entity(entity_object.bounding_rectangle.list,
                                                     border_entity, window_kind='百叶')
                ...
                for drawing_type, info_dict in special_drawing_dict.items():
                    ...
                    if drawing_type == drawing_type_architecture.DOOR_WINDOW_DAYANG:
                        ...
                        for lm_window_obj in lm_window_obj_list:
                            if window_number in lm_window_obj.window_number:
                                self._get_window_altitude(file_id, dy_border_entity, lm_window_obj)   # 获取窗台高度
                                self._get_window_open_ways(file_id, dy_border_entity, lm_window_obj)  # 获取规格尺寸、开启方式，开启方向、分隔方式、
                                break
    ...
    def _get_window_open_ways(self, file_id, border_entity_info, lm_window_obj):
        ...
        all_text_info = border_entity_info.border_text_info[TextType.ALL]
        all_text_info = [text for text in all_text_info if
                         text.extend_message.replace('.', '').isdigit() and Iou_temp(ext_bbox, text.bbox.list)]
        ...
        for text in all_text_info:
            ...
            if text_message > 50 and ((Iou_temp(ext_bbox1, text_bbox) and ((text_bbox[1] + text_bbox[3]) / 2 > bbox[3]))
                                      or (Iou_temp(ext_bbox3, text_bbox) and (
                            (text_bbox[1] + text_bbox[3]) / 2 < bbox[1]))):
            ...
            if text_message > 50 and ((Iou_temp(ext_bbox2, text_bbox) and ((text_bbox[0] + text_bbox[2]) / 2 > bbox[2]))
                                      or (Iou_temp(ext_bbox4, text_bbox) and (
                            (text_bbox[0] + text_bbox[2]) / 2 < bbox[0]))):
        ...
        for text in all_text_info:
            ...
            if division_width and txt_msg not in division_width and text_bbox[1] == division_width[0][1] and text_bbox[
                3] == division_width[0][3]:
                division_width.append(txt_msg)
            if division_height and txt_msg not in division_height and text_bbox[0] == division_height[0][0] and \
                    text_bbox[2] == division_height[0][2]:
        ...
        for contour in seg_contours:
            ...
            for arrow in arrow_list:
                ...
                if get_contours_iou(contour, arrow_cnt) > 0.5:
                    self.window_open_ways.append("推拉")
            ...
            self.window_open_ways.append(None)
            self.window_open_direction.append(None)
        print('Note: 百叶Window[{}]\'s open_ways is [{}], open_direction is [{}],\t\r\ndivision is [{}] win_size:[{}x{}]'.format(
                self.window_number, self.window_open_ways, self.window_open_direction, self.division,
                self.lm_window_width, self.lm_window_height))
            ...
            if self.window_number in dw_size_material:
                ...
                if len(split_msg) > 1:
                    ...
                    if re.search('[0-9]', self.glass_material):
                        self.glass_material = self.glass_material.replace('中空', '').replace('玻璃', '').lower()
