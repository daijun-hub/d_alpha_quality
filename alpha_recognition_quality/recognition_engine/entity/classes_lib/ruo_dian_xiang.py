...
class RuoDianXiang(ClassifiedEntity):
    chinese_name = "弱电箱"

    def __init__(self, entity_object: Entity, border_entity: BorderEntity) -> None:
        ClassifiedEntity.__init__(self, entity_object)

        self.chinese_name = "弱电箱"
    ...
    def get_height(self, file_id, border_entity_info):
        ...
        if not install_way_list:
            return False
        # 唯一强电箱文本,并纵向外扩
        keyword = [text for text in all_text_info if re.search('家居配线箱', text.extend_message)]
        if not keyword:
            return False
        ...
        for install_height in install_height_list:
            ...
            if point_center[0] in range(install_way_bbox_extend[0], install_way_bbox_extend[2]) and point_center[
                ...
                print(f'找到住户家居配线箱的安装高度{install_height_text}')
