...
class ZhuanJiaoChuang(ClassifiedEntity):
    ...
    def __init__(self, entity_object: Entity, border_entity: BorderEntity) -> None:
        ...
        self.chinese_name = "转角窗"
        self.window_is_outside = judge_outside_window_entity(entity_object.bounding_rectangle.list,
                                                      border_entity,
                                                      window_kind='转角窗')
        self.window_fire_resistance_level = judge_window_fire_proof_entity(entity_object.bounding_rectangle.list,
                                                                    border_entity,
                                                                    window_kind='转角窗')
        self.window_number = window_numbering_entity(entity_object.bounding_rectangle.list,
                                              border_entity,
                                              window_kind='转角窗')
        self.window_width = window_width_height_entity(entity_object.bounding_rectangle.list,
                                                border_entity,
                                                window_kind='转角窗')[0]
        self.window_height = window_width_height_entity(entity_object.bounding_rectangle.list,
                                                 border_entity,
                                                 window_kind='转角窗')[1]
                    ...
                    if drawing_type == drawing_type_architecture.DOOR_WINDOW_DAYANG:
                        ...
                        for lm_window_obj in lm_window_obj_list:
                            ...
                            if window_number in lm_window_obj.window_number:
                                ...
                                self._get_window_open_ways(file_id, dy_border_entity, lm_window_obj, is_flip)  # 获取规格尺寸、开启方式，开启方向、分隔方式、
                                break
    ...
    def _get_window_open_ways(self, file_id, border_entity_info, lm_window_obj, is_flip):
        ...
        # 给扇按质心排序，先左后右，先上后下
        seg_contours, flip_orders = sort_contour(seg_contours, is_flip=is_flip)
        ...
        print('Note: 转角窗Window[{}]\'s open_ways is [{}], open_direction is [{}],\t\r\ndivision is [{}] win_size:[{}x{}]'.format(
