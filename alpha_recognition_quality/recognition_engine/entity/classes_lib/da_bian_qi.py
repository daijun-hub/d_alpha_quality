...
# 分类类型
class DaBianQi(ClassifiedEntity):
    chinese_name = "大便器"

    def __init__(self, entity_object: Entity, border_entity: BorderEntity) -> None:
        ClassifiedEntity.__init__(self, entity_object)

        self.chinese_name = "大便器"
    ...
    def get_basis_point_line(self, border_entity):
        ...
        if len(contours_) > 0:
            ...
            if len(new_contours) > 0:
                ...
                # 目前图纸中大便器在空间中会有两种情况：横放和竖放,一般以大便器中心线和长的边平行，但洗脸盆是反着的
                if h >= w:
            ...
            else:  # 没有得到大便器contours
                w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
                x, y = [int(bbox[0] + 0.5 * w), int(bbox[1] + 0.5 * h)]
