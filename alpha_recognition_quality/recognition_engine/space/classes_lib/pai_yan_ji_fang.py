...
class PaiYanJiFang(Space):
    chinese_name = "排烟机房"

    def __init__(self, space_object: Space, border_entity: BorderEntity) -> None:
        Space.copy(self, space_object)
        self.chinese_name = "排烟机房"
    ...
    def _get_floor(self, border_entity):
        ...
        if drawing_type == DrawingTypeArchi.JIFANG:
            return '机房层'
        elif drawing_type == DrawingTypeArchi.UNDERGROUND:
            return '地下层'
    ...
    def _get_related_wall(self, border_entity):
        ...
        emission_room_cnt_ext = expand_contour(self.contour.contour, 10)
        for nobear_wall in nobear_wall_list:
            if get_contours_iou(emission_room_cnt_ext, nobear_wall.contour.contour) > 0:
    ...
    def _get_related_door(self, border_entity):
        ...
        for label in map_label:
            ...
            for door in door_list:
                if get_contours_iou(emission_room_cnt_ext, get_contour_from_bbox(door.bounding_rectangle.list)) > 0:
    ...
    def _get_related_hole(self, border_entity):
        ...
        for hole in hole_list:
            if get_contours_iou(emission_room_cnt_ext, get_contour_from_bbox(hole.bounding_rectangle.list)) > 0:
