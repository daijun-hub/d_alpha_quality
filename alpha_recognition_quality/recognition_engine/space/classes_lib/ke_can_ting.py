...
class KeCanTing(Space):
    chinese_name = "客餐厅"

    def __init__(self, space_object: Space, border_entity: BorderEntity) -> None:
        Space.copy(self, space_object)
        self.chinese_name = "客餐厅"
    ...
    def _get_related_wall(self, border_entity):
        ...
        living_cnt_ext = expand_contour(self.contour.contour, 10)
        for nobear_wall in nobear_wall_list:
            if get_contours_iou(living_cnt_ext, nobear_wall.contour.contour) > 0:
    ...
    def _get_related_door(self, border_entity):
        ...
        for label in map_label:
            ...
            for door in door_list:
                if get_contours_iou(living_cnt_ext, get_contour_from_bbox(door.bounding_rectangle.list)) > 0:
    ...
    def _get_related_window(self, border_entity):
        ...
        for label in map_label:
            ...
            for window in window_list:
                if get_contours_iou(living_cnt_ext, get_contour_from_bbox(window.bounding_rectangle.list)) > 0:
    ...
    def _get_related_hole(self, border_entity):
        ...
        for hole in hole_list:
            if get_contours_iou(living_cnt_ext, get_contour_from_bbox(hole.bounding_rectangle.list)) > 0:
