...
class LuTai(Space):
    ...
    def _get_related_wall(self, border_entity):
        ...
        terrace_cnt_ext = expand_contour(self.contour.contour, 10)
        for nobear_wall in nobear_wall_list:
            if get_contours_iou(terrace_cnt_ext, nobear_wall.contour.contour) > 0:
    ...
    def _get_related_door(self, border_entity):
        related_door = []
        door_type_list = ['单开门', '双开门', '子母门']
        terrace_cnt_ext = expand_contour(self.contour.contour, 10)
        for door_type in door_type_list:
            door_list = border_entity.entity_object_dict.get(door_type, [])
            for door in door_list:
                if get_contours_iou(terrace_cnt_ext, get_contour_from_bbox(door.bounding_rectangle.list)) > 0:
    ...
    def _get_related_window(self, border_entity):
        ...
        for window_type in window_type_list:
            window_list = border_entity.entity_object_dict.get(window_type, [])
            for window in window_list:
                if get_contours_iou(terrace_cnt_ext, get_contour_from_bbox(window.bounding_rectangle.list)) > 0:
    ...
    def _get_related_hole(self, border_entity):
        ...
        for hole in hole_list:
            if get_contours_iou(terrace_cnt_ext, get_contour_from_bbox(hole.bounding_rectangle.list)) > 0:
