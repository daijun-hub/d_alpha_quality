...
# TODO: 衣帽间涉及空间细划分，目前不对非封闭衣帽间进行处理，主要是和卧室联通，作为一个大空间来用
class YiMaoJian(Space):
    chinese_name = "衣帽间"

    def __init__(self, space_object: Space, border_entity: BorderEntity) -> None:
        Space.copy(self, space_object)
        self.chinese_name = "衣帽间"
    ...
    def _get_related_wall(self, border_entity):
        ...
        cloakroom_cnt_ext = expand_contour(self.contour.contour, 10)
        for nobear_wall in nobear_wall_list:
            if get_contours_iou(cloakroom_cnt_ext, nobear_wall.contour.contour) > 0:
    ...
    def _get_related_door(self, border_entity):
        ...
        for label in map_label:
            ...
            for door in door_list:
                if get_contours_iou(cloakroom_cnt_ext, get_contour_from_bbox(door.bounding_rectangle.list)) > 0:
    ...
    def _get_related_window(self, border_entity):
        ...
        for label in map_label:
            ...
            for window in window_list:
                if get_contours_iou(cloakroom_cnt_ext, get_contour_from_bbox(window.bounding_rectangle.list)) > 0:
    ...
    def _get_related_hole(self, border_entity):
        ...
        for hole in hole_list:
            if get_contours_iou(cloakroom_cnt_ext, get_contour_from_bbox(hole.bounding_rectangle.list)) > 0:
    ...
    def _is_close_space(self, border_entity):
        ...
        # 判断衣帽间是否和某一个卧室完全重合，则该衣帽间是一个开敞衣帽间
        bedroom_list = border_entity.space_object_dict.get('卧室', [])
        for bedroom in bedroom_list:
            if get_contours_iou(self.contour.contour, bedroom.contour.contour) > 0.9 and \
                    get_contours_iou(bedroom.contour.contour, self.contour.contour) > 0.9:
