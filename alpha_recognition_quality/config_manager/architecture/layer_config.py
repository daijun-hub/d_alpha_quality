...
class LayerConfig(Enum):
    ...
    ENTITY_LAYER_DICT = {
        ...
        "door": {  # 门
            ...
            "ignore_word": BASIC_IGNORE_WORDS["basic"] + ["立面门窗", "人防", "0-DOOR\.T", "WINDOW[-_]BLIN",
                                                          "开启", "门套", "A[-_]EQPM[-_]WIND", "阀门", "IBS-WINDOW", "立面-门窗开启"],
        },
        "arrow": {  # 箭头
            "layer_sub": ["DIM", "SYMB", "ANNO", "符号标注", "AD-SIGN", "DIM[-_]SYMB", "A[-_]ANNO[-_]SYMB",
                          ...
                          "A_SIGN_ARRO(上下行箭头线)", "A_SYMB_DRCT(坡向线)", "AD-SIGN", "A-INDEX", "A-注释-符号", "^DS$", "门窗开启"],
        ...
        "elevator_stair": {  # 电梯厢/楼梯（住宅平面图）
            ...
            "ignore_word": list(set(BASIC_IGNORE_WORDS["basic"]) - {"LINE"}) +
                           ['SMAL', 'BALC', 'ELEC', 'LIHT', 'CLNG', 'FIRE', 'HYDR', 'KICH', 'ASSI', 'MECH', 'PLOM',
                            'TOLT', 'PRKG', 'SYMB', "RAIL", "^FIT$", "STAIR-栏杆", "CAR", "TACC", "洁具", "A[-_]EQPM", "JACK"],
        },
        "parking": {  # 车位
        ...
        "fire_hydrant": {  # 消火栓
            "layer_sub": ["消火栓", "消防箱", "消火箱", "消防", "XHS", "HYDT_BOX", "HYDT", "XHSX", "HYDRANT", "EQPM-FIRE", "HYDR",
                          "^0P-ACC-FH$", "^0P-EQUIP-FH$", "FHBX", "FIRE-FH", "灭火器", "消栓[-_]栓箱", "YCS[-_]消防管.*配件", "S-HHSH"],
            "ignore_word": list(set(BASIC_IGNORE_WORDS["basic"]) - {"标注"}) + ["立管", "PIPE", "阀门", "风管", "法兰", "风口", "消防分析", "消防出口"],
        },
        "emergency_door": {  # 人防
            "layer_sub": ["WINDOW", "WIN", "door", "门", "人防"],
            "ignore_word": BASIC_IGNORE_WORDS["basic"] + ['人防-计入单体、非人防面积', "井"] +
                           ["分区", "0-DOOR\.T", "WINDOW[-_]BLIN", "开启范围", "人防集水坑", "人防经济指标", "门套", "A[-_]EQPM[-_]WIND", "阀门", "IBS-WINDOW", "立面-门窗开启"],
        },  # 过滤人防分区
        "wall": {  # 墙
        ...
        "pillar": {  # 柱子
            "layer_sub": ["柱", "COLU", "COLUMN", "COLS", "S-Col", "S-Col_hatch", "S-WC", "CLOS", "S_CL$"],  # 华住图纸柱子图层添加S_CL
            "ignore_word": BASIC_IGNORE_WORDS["basic"] + ["柱帽", "填充", "A-COLU-CONC-HATC", 'A-COLS-C-S', 'S-墙柱-HATCH'],
        },
        "pillar_cap": {  # 柱帽
            "layer_sub": ["柱帽", "Z帽"],
            "ignore_word": BASIC_IGNORE_WORDS["basic"],
        ...
        "axis_net": {
            "layer_sub": ["建-标注", "建-轴线", "axis", "GRID", "DOTE", "轴网", "轴线", "轴号", "C[-_]标注"],     # 邀测图纸轴网图层新加，且设计师描述DOTE、轴网为轴网常用图层
            "ignore_word": [],    #list(set(BASIC_IGNORE_WORDS["basic"]) - {"标注", "尺寸"}),
        },
        "elevator_box": {  # 电梯厢/楼梯
            "layer_sub": ["EVTR", "Stair", "STAIR", "Lift", "LIFT", "电梯", "楼梯", "梯", "FIT", "ELEV", "elevator", "STRS",
        ...
        "segment": {  # 住宅平面空间识别图层
            ...
            "ignore_word": list(set(BASIC_IGNORE_WORDS["basic"]) - {"LINE", "线"}) + [
                ...
                "A-FLOR-UNDR", "J[-_]FLOR[-_]DICH.*建筑沟", "A-HOLE-FLOR", "A[-_]1[-_]FLOR", "A[-_]FLOR"]
        },  # "^A-OVER$" 会用到空调板的边界
        ...
        "air_conditioner": {  # 空调
            "layer_sub": ["空调", "^空_$", "Aircontroe", "^AC$", "Aircondition", "EQPM-MECH", "EQPM-SMAL", "M_AC", "KT",
                          "FLOR-OVHD", "LA-plan-smal", "RS-A", "^A-AC$", "A[-_]FLOR[-_]AIRC", "^A[-_]EQPM[-_]WIND$", "AC"],  # A-EQ
        ...
        "elevation_handrail": {  # 立面栏杆
            "layer_sub": ["栏杆", "HRAL", "handrail", "RAIL", "HDWR", "SC-LINE", "hdrl", "栏杆", "BALCONY",  "HANDRAIL",
                          "阳台", "Rail", "HRAL", "blcn", "Hdrl", "HANDRA", "AE-HDWR", "护栏", "A_HDWR", "RALL"],  # "STAIR"暂时去掉
        ...
        "plan_handrail": {  # 平面栏杆（既用于空间分割，也用于构件）
            ...
            "ignore_word": BASIC_IGNORE_WORDS["basic"] + ['^GX阳台$', '阳台排水', "A-Blcn"],
        },
        "pave": {  # 铺装
            "layer_sub": ["pave"],
        ...
        "elevation_window": {  # 立面窗
            "layer_sub": ["wind", "lmw", "WIN", "窗户", "DRWD", "窗线", "窗边", "门窗", "D&W", "立面开启扇",
                          "A[-_]ELEV[-_]METL", "立面.*窗", "墙", "WALL"],  # 墙是为了针对A-墙体图层
            "ignore_word": BASIC_IGNORE_WORDS["basic"] + ["sher", "WINDOW[-_]BLIN", "开启", "A[-_]EQPM[-_]WIND",  "A[-_]ELEV[-_]WIND[-_]GRAY"],
        },
        "elevation_window_open_line": {  # 立面窗开启线
            "layer_sub": ["门窗开启","窗户开启", "立面-材料填充", "大样-填充", "^A[-_]ELEV[-_]RIP$", "^A[-_]SECT[-_]THIN$", "A[-_]PATN[-_]MATE.*图案填充", "PUB_HATCH", "PUB-HATCH"],
            "ignore_word": BASIC_IGNORE_WORDS["basic"],
        },
        "elevation_mark": {
            "layer_sub": ["A-Anno-Levl", "P-ROAD-ELEV", "DIM_ELEV", "ELEV_DIM", "标高", "GCD", "A[-_]SYMB[-_]LEVL.*相对标高",
                          ...
                          "A-Elv(建筑标高)", "A_N_DIM_ELEV", "建-符号", "A-ELEVEL", "A-标注-楼层标高标注"],
            "ignore_word": ["^地面标高$"]
        },
        "structure": {
        ...
        "second_third_space": {  # 雨棚边界图层
            ...
            "ignore_word": ["WINDOW[-_]BLIN", "A-STAIR-电梯", "A-大样-看线"],
        },
        "indoor_access": {
            "layer_sub": ["stair", "梯"],
        ...
        "annotation_line": {  # 引线图层、箭头图层
            "layer_sub": ["DIM", "IDEN", "ANNO", "SYMB", "LEAD", "0[-_]坐标标注", "NO\..*编号", "TEXt", "-TEXT",
                          ...
                          "A_INDX(索引)", "A-索引标注",
                          "A-引出标注", "A-N-DIM-LEAD", "J-符号", ".*符号$"],
            "ignore_word": ["PUB", "标高", "红线", "流线", "坐标", "尺寸", "中线", "ELEV", "^TWT[-_]SYMB$", "FROM",
                            "FORM", "GRID", "A-COND-IDEN-TEXT", "LEVL", "LEVEL", "P[-_]FS[-_]EQPM[-_]TEXT",
                            "A[-_]DIM[-_]AXIS", "A[-_]ANNO[-_]NOTE", "A[-_]ANNO[-_]DIMS", ".*孔洞符号$"],
        },
        "engineering_work_table_line": {
            "layer_sub": ["HS-A-说明文字", '通用-轴网标注', "ANNO-TABS", "底框", "0-TTLB", "A-BORD", "TK",
                          "AAD-A1$0$AAD-FR-LINE", "图框线", "A-文字-功能", "PUB_TAB", "表", "^A-文字-平面说明$",
                          "^A-云图图框$", "PUB_TEXT", "DIM", "A-大样-看线", "平面饰线", "平面[-_]饰线", "A-大样-填充", "A-活动层-引用", "A-标注-尺寸标注"],
            "ignore_word": []
        },
        # "plane_decorative_line": {
        #     "layer_sub": ["平面饰线"],
        #     "ignore_word": []
        # },
        "filter_road_layer": {
        ...
        "kitchen_exhaust_pipe": {  # 厨房排烟管道
            "layer_sub": ["AE-HOLE", "管道", "A[-_]FLOR[-_]SPCL", "A[-_]FLOR[-_]FLUE", "A[-_]烟道风道",
                          ...
                          "A-(平面-)?烟道", "EQU_厨卫设备", "EQUI-FLUE", "C-厨房设备", "A-厨卫器具", "C-厨卫", "建-平-风道烟道",
                          "排风道", "A-FURN-排风道", "QN-J-厨卫", "E[-_]ED2[-_]细", 'S-墙柱-HATCH', "A-平面-楼板开洞"],
            "ignore_word": list(set(BASIC_IGNORE_WORDS["basic"]) - set(["道"])),
        },
        ...
        "cut_off_line": {  # 截断线
            "layer_sub": ["DIM[-_]SYMB", "A[-_]ANNO[-_]IDEN", "DIM[-_]SYMB[-_]剖切标注", "SYMB_折断线及跑向_A", "0A-A-SYMB", "A-注释-符号", "A_N_DIM_SYMB"],
            "ignore_word": BASIC_IGNORE_WORDS["basic"],
        },
        ...
        "san_shui": {   #散水
            ...
            "ignore_word": BASIC_IGNORE_WORDS["basic"] + ["A-大样-看线"],
        },
        "fen_ji_shui_qi": { #分集水器
            "layer_sub": ["A[-_]分集水器", "集水器", "Qh", "采暖"],
            "ignore_word": BASIC_IGNORE_WORDS["basic"] + [],
        }
    }
    ...
    COMBINATION_EXCLUDE_LAYERS_INDOOR = [
        ...
        "annotation", "axis_grid", "axis_net", "decorative"
    ...
    CLASSIFICATION_EXCLUDE_LAYERS = [
        ...
        "elevation_window_open_line", "annotation", "axis_grid", "engineering_work_table_line", "axis_net", "decorative"
    ]

    # 分图层打印配置
    INDOOR_DRAWING_PRINT_LAYER_SEPARATE_SET = {
        'set_1': ['pipe', 'floor_drain', 'door', 'elevator_door', 'window', 'air_conditioner', "water_pit", "fen_ji_shui_qi"],
        ...
        'set_3': ['floor_drain_mix', 'air_conditioner_mix', 'fire_hydrant', 'elevator_box', 'pipe', 'floor_drain',
                  'air_conditioner', 'elevator_stair', 'washbasin', 'closestool', 'diamond_bath', "kitchen_exhaust_pipe", 'yu_liu_kong_dong'],
