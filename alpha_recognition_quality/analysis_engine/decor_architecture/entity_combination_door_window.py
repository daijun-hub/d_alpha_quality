    ...
    for layer, bbox_list in layer_bbox_list.items():
        ...
        if layer in ['floor_drain']:
            ...
            combine_bbox_contour = combine_new_indoor(layer, origin_entity_info, image_size, ratio_w_h, ext_margin,
                                                      ...
                                                      small_close_kernel=True, major='architecture')
            combine_bbox = [info[0] for info in combine_bbox_contour]
        ...
        #                     PIPE_SIZE * ratio]
            ...
            for bbox in temp_combine_bbox:
                ...
                bbox_seperation = separation_combination(bbox_img, ratio, major='architecture')
                del bbox_img
                gc.collect()
            ...
            combine_bbox = [bbox for bbox in temp_combine_bbox if
                ...
                drawing_type in [DrawingType.DOOR_WINDOW_DAYANG]:
            ...
            combine_bbox = combine(bbox_list, image_size)
        elif layer in ['door', 'window', 'elevator_door'] and drawing_type not in [DrawingType.DOOR_WINDOW_DAYANG]:
            # combine_bbox_contour = combine_new_indoor(layer, origin_entity_info, image_size, ratio_w_h, ext_margin,
...
def filt_bbox(layer, bbox_list, border_entity_info, drawing_type, crop_entity_image, entity_save_path):
    ...
    for bbox in bbox_list:
        ...
        if layer not in LayerConfig.CLASSIFICATION_EXCLUDE_LAYERS.value and crop_entity_image:
            if layer not in ['window', 'elevator_door', 'door', 'emergency_door', 'elevation_window']:
                continue
            # if border_info:
...
def run(border_name, border_entity_info, result_path, drawing_type, border_info=[], dwg_name="", rule_index=None,
    ...
    for layer, bbox_list in border_entity_bbox_combined.items():
        ...
        if layer in ['dayang_handrail', 'elevation_handrail']:
            border_entity_bbox_dict[layer] = bbox_list
            continue

        if drawing_type in [DrawingType.INDOOR, DrawingType.INDOOR_BATHROOM, DrawingType.INDOOR_FIRST_FLOOR,
                            DrawingType.INDOOR_FIRST_FLOOR_ACCESS, DrawingType.SIDE_ELEVATION,
                            DrawingType.SECOND_THIRD_FLOOR, DrawingType.BINANCENG] \
                and layer in LayerConfig.COMBINATION_EXCLUDE_LAYERS_INDOOR.value + LayerConfig.LAYERS_WITH_SLOPE_LINE_REVISED.value + \
                LayerConfig.DASH_LINE_REVISED.value:
        ...
        if drawing_type in [DrawingType.UNDERGROUND, DrawingType.SITE_PLAN_ROAD, DrawingType.UNDERGROUND_BASEMENT,
                            ...
                            DrawingType.FIRST_FLOOR_SITE_PLAN] and \
                layer in LayerConfig.COMBINATION_EXCLUDE_LAYERS_UNDERGROUND_AND_SITEPLAN.value + \
                ['pillar', 'special_pillar'] + LayerConfig.LAYERS_WITH_SLOPE_LINE_REVISED.value + \
                LayerConfig.DASH_LINE_REVISED.value:
            bbox_list = list(set([tuple(bbox) for bbox in bbox_list]))
            bbox_list = [list(i) for i in bbox_list]
            border_entity_bbox_dict[layer] = bbox_list
    ...
    if drawing_type in [DrawingType.INDOOR, DrawingType.INDOOR_FIRST_FLOOR, DrawingType.INDOOR_FIRST_FLOOR_ACCESS,
                        DrawingType.UNDERGROUND, DrawingType.JIFANG, DrawingType.WUMIAN, DrawingType.BINANCENG]:
        # 获取像素相交分离构件并画粗黄线
    ...
    # 对柱子画黄框，for debug
    if DEBUG_MODEL and 'pillar' in border_entity_bbox_dict:
    ...
    # 进行引线系统合并
    main_branch_text_list, _ = get_annotation_group(border_entity_info, result_path, img_anno=None)
